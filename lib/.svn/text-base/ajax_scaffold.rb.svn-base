module AjaxScaffold # :nodoc:
  class ScaffoldColumn
    ...
    def initialize(klass, options)
      @name = options[:name]
      @eval = options[:eval].nil? ? "#{Inflector.underscore(klass.to_s)}." +
                                    "#{@name}" : options[:eval]
      @label = options[:label].nil? ? Inflector.titleize(@name) \
                                    : options[:label]
      @sortable = options[:sortable].nil? ? true : options[:sortable]
      @sort_sql = options[:sort_sql].nil? ? "#{klass.table_name}.#{@name}" \
                                        : options[:sort_sql] unless !@sortable
  ...
  module Controller
    ...
    def update_params(options)
      ...
      session[@scaffold_id] ||= {:sort => options[:default_sort],
  ...
  module Helper
    ...
    def format_column_csv(column_value, sanitize = true)
      '"' + format_column(column_value, sanitize).gsub(/"/,'\"') + '"'
    end
    ...
    def pagination_ajax_links(paginator, params)
      pagination_links_each(paginator, {}) do |n|
        link_to_remote n,
           ...
           "#{loading_indicator_id(params.merge(:action => 'pagination'))}');",
    ...
    def column_class(column_name, column_value, sort_column, class_name = nil)
      ...
      class_attr += "sorted " \
                           if (!sort_column.nil? && column_name == sort_column)
    ...
    def loading_indicator_tag(options)
      image_tag "indicator.gif",
                :style => "display:none;",
                :id => loading_indicator_id(options),
                :alt => "loading indicator",
                :class => "loading-indicator"
    end

    # The following are a bunch of helper methods to produce the common
    # scaffold view id's
    ...
    def loading_indicator_id(options)
      if options[:id].nil?
        ...
        "#{options[:scaffold_id]}-#{options[:action]}-" +
        "#{options[:id]}-loading-indicator"
      end
    end
